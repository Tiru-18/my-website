function init_onlyyou() {
    ...
    // Set title
    var title = (typeof(start_content['chase_title']) !== 'undefined' && start_content['chase_title'] !== '') ? start_content['chase_title'] : 'Will You Be Mine Next time Muuu💗?';
    ...
    var content = (typeof(start_content['chase_text']) !== 'undefined' && start_content['chase_text'] !== '') ? start_content['chase_text'] : `
        ...
        ❤️ My Lifeline, the love that keeps me alive ❤️function init_onlyyou() {
    // Adjust the height of the divs to window height plus some offset
    $("#div_onlyyou").css({"height": $(window).height() + 260 + "px"});
    $("#div_oy_inner").css({"height": $(window).height() + 260 + "px"});
...
function playBackgroundMusic() {
    // Assuming music_json is available and contains the audio URL
    var audioSource = music_json.m_online_url; // Dynamic audio source from music_json

    // Create an audio element dynamically and set the source from music_json
    var audio = new Audio(audioSource); // Use the dynamic URL from music_json
...
function random_img_as() {
    ...
    return random_img;
        ...
        And yet... I am just something to you 😊,but me lost forever in you.
